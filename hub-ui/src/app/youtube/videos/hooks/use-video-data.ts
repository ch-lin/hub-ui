...
export function useVideoData({
  ...
  const fetchItems = useCallback(
    async (mode: "available" | "upcoming" | "all" | "deleted", channelId: string, page: number, size: number, options: { isManualRefresh?: boolean; showLoading?: boolean } = {}) => {
      ...
      try {
        ...
        // Clear selection on page/filter change to avoid stale selections
  ...
  const handleBatchDownload = useCallback(async (videoIds: string[]) => {
    ...
    setDownloadingItems((prev) => {
      const newSet = new Set(prev);
      videoIds.forEach(id => newSet.add(id));
      return newSet;
    });
    console.info(`[Videos Tracking] Action: Submitting batch download job for ${videoIds.length} videos`);
    
    // Optimistic UI update: Immediately set status to PENDING so dropdown reflects it
    setItems((prev) => prev.map((item) => videoIds.includes(item.videoId) ? { ...item, status: "PENDING" } : item));
    
    const toastId = toast.loading(`Submitting download job for ${videoIds.length} video(s)...`);
    const startTime = performance.now();

    try {
      const result = await videoApi.downloadVideos(videoIds);

      const createdTasksCount = result.data?.createdTasks;
      if (typeof createdTasksCount === "number" && createdTasksCount > 0) {
        const message = `Successfully created ${createdTasksCount} download task(s).`;
        toast.success(message, { id: toastId });
        console.info(`[Videos Tracking] Batch download job submitted successfully in ${(performance.now() - startTime).toFixed(0)}ms.`);
      ...
      const message = err instanceof Error ? err.message : "An unknown error occurred while submitting the batch download job.";
      toast.error(`Batch download failed: ${message}`, { id: toastId });
      console.error(`[Videos Tracking] Error submitting batch download:`, err);
      
      // Revert optimistic update and remove from polling queue on error
      setItems((prev) => prev.map((item) => videoIds.includes(item.videoId) ? { ...item, status: "FAILED" } : item));
      setDownloadingItems((prev) => {
        const newSet = new Set(prev);
