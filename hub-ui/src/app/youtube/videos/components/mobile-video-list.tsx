        ...
        <div className="relative w-full aspect-video bg-muted">
          <div className="absolute top-2 left-2 z-10">
            <Checkbox 
              ...
              className="border-white data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground shadow-sm w-5 h-5 rounded-sm bg-black/20"
            />
          </div>
          <Image 
            src={imgSrc} 
          ...
          <TooltipProvider>
            <Tooltip delayDuration={300}>
              <TooltipTrigger asChild>
                <div className={item.status === 'PENDING' ? 'cursor-help' : ''}>
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button 
                        size="icon" 
                        variant={item.status === 'DOWNLOADED' ? 'secondary' : 'default'} 
                        className={`h-12 w-12 rounded-full shadow-md transition-transform active:scale-95 ${isDownloading || ['PENDING', 'DOWNLOADING', 'DELETED', 'IGNORE'].includes(item.status) ? 'pointer-events-none' : ''}`} 
                        disabled={isDownloading || ['PENDING', 'DOWNLOADING', 'DELETED', 'IGNORE'].includes(item.status)}
                      >
                        {isDownloading || ['PENDING', 'DOWNLOADING'].includes(item.status) ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Download className="h-5 w-5" />}
                      </Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                      <AlertDialogHeader>
